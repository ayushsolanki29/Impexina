...
export default function LoadingSheetPage() {
  ...
  useEffect(() => {
        ...
        try {
          ...
          if (response.data.success && response.data.data.length > 0) {
            ...
            const exactMatch = response.data.data.find(
              (s) => s.name.toLowerCase() === clientName.trim().toLowerCase(),
            );
            if (exactMatch) {
              setClientId(exactMatch.id);
              // Only update name if it differs (to avoid unnecessary re-renders)
              if (exactMatch.name !== clientName) {
                setClientName(exactMatch.name);
              }
              setClientSuggestions([]); // Clear suggestions as we found a match
            }
          }
        } catch (error) {
    ...
    // Debounce to allow user to finish typing and avoid too many API calls
    const timer = setTimeout(autoLink, 500);
  ...
  const fetchContainerData = async () => {
    try {
      ...
      const response = await API.get(
        `/loading-sheets/container/${containerId}`,
      );
  ...
  const loadSheet = (sheet) => {
    ...
    if (urlClientName && sheet.clientName === urlClientName) {
      // Pre-select this client if it matches the URL parameter
  ...
  const handlePhotoUpload = async (index, event) => {
    ...
    try {
      ...
      const encodedMark = encodeURIComponent(markForUpload);

      const response = await API.post(
        `/loading-sheets/upload-photo?containerCode=${cCode}&shippingMark=${encodedMark}`,
        formData,
        {
  ...
  const fetchMarkSuggestions = async (search) => {
    ...
    try {
      const response = await API.get(
        "/loading-sheets/suggestions/shipping-marks",
        {
          params: { search },
        },
      );
  ...
  const handleContainerStatusChange = async (newStatus) => {
    try {
      const response = await API.patch(`/containers/${containerId}`, {
        status: newStatus,
      });
      if (response.data.success) {
  ...
  const handleSheetStatusChange = async (newStatus) => {
    ...
    try {
      const response = await API.patch(
        `/loading-sheets/${activeSheet.id}/status`,
        { status: newStatus },
      );
      if (response.data.success) {
  ...
  const stats = loadingSheets.reduce(
    (acc, sheet) => {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      default:
        return "bg-amber-100 text-amber-800 border-amber-200"; // DRAFT
    }
  };
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 h-24 animate-pulse"
              >
          ...
          <div className="mb-6 flex gap-2 overflow-hidden animate-pulse">
            {[1, 2, 3, 4, 5].map((i) => (
              <div
                key={i}
                className="w-36 h-16 bg-slate-200 rounded-lg flex-shrink-0"
              ></div>
            ))}
          </div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-5 transition-all hover:shadow-md hover:border-blue-100 group">
            ...
            <div>
              <div className="text-[10px] font-medium text-slate-400 uppercase tracking-widest mb-1">
                Total Items
              </div>
        ...
        <div className="bg-white rounded-xl shadow-lg mb-6 overflow-hidden">
          {/* Status Header Bar */}
          <div
            className={`px-6 py-4 flex justify-between items-center border-b ${getStatusColor(
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm font-bold uppercase tracking-widest hover:scale-105 transition-transform cursor-default">
                {activeSheet?.status || "DRAFT"}
              </span>
            </div>
          ...
          <div className="p-6">
            <div className="grid grid-cols-2 gap-6 mb-6">
              ...
              <div>
                ...
                <div className="relative">
                  <div className="relative">
                    <Users
                      className={`absolute left-3 top-3.5 w-5 h-5 z-10 ${clientId ? "text-green-500" : "text-slate-400"}`}
                    />
                    <input
                      type="text"
                      value={clientName}
                    ...
                    {clientId && (
                      <div className="absolute right-3 top-3.5 flex items-center gap-1.5">
                        <span className="text-[10px] font-bold text-green-600 bg-green-100 px-1.5 py-0.5 rounded uppercase tracking-wider">
                          Linked
                        </span>
                        <button
                          onClick={() => {
            ...
            {/* Items Table */}
            <div className="border border-slate-100 rounded-2xl overflow-scroll shadow-sm">
              <table className="w-full text-xs border-collapse">
                          ...
                          {item.photo ? (
                            <div className="relative group w-9 h-9">
                              <img
                                ...
                                onError={(e) => {
                                  console.error("Image load error:", getImageUrl(item.photo), item.photo);
                ...
                <tfoot>
                  <tr className="bg-slate-50 border-t-2 border-slate-300">
                    <td
                      colSpan={5}
                      className="px-4 py-3 text-right font-bold text-slate-600 uppercase tracking-wider text-xs"
                    >
                      Totals
                    </td>
