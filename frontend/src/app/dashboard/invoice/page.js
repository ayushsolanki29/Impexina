...
import {
  FileText, Search, Calendar, MapPin, Loader2,
  ArrowRight, CheckCircle2, Circle, DollarSign,
...
export default function InvoiceListPage() {
  ...
  const handleExportAll = async () => {
    try {
      ...
      const response = await API.get("/invoice/export/all", {
        responseType: 'blob'
      });
      ...
      link.href = url;
      link.setAttribute('download', `all_invoices_${new Date().toISOString().slice(0, 10)}.xlsx`);
    ...
    <div className="min-h-screen bg-slate-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Commercial Invoice</h1>
            <p className="text-slate-500 mt-1">Create and manage invoices for your containers</p>
          </div>
          ...
          <div className="flex flex-col lg:flex-row gap-4 items-center">
            <div className="relative flex-1 w-full">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl shadow-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all outline-none text-sm font-medium"
              />
            </div>
            ...
            <div className="flex items-center gap-3 bg-white border border-slate-200 px-4 py-2.5 rounded-xl group focus-within:ring-2 focus-within:ring-emerald-500 transition-all w-full lg:w-auto shadow-sm">
        ...
        {loading && containers.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {containers.map((container) => (
              <div
                key={container.id}
                onClick={() => router.push(`/dashboard/invoice/${container.id}`)}
                className="group bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-xl hover:border-emerald-200 transition-all cursor-pointer overflow-hidden flex flex-col"
              >
                  ...
                  <div className="space-y-2">
                    ...
                    {container.invoice && (
                      <div className="flex items-center gap-2 text-sm text-emerald-600 font-bold">
                        ...
                        <span>${container.invoice.totalAmountUsd?.toFixed(2) || '0.00'}</span>
                      </div>
                    )}
                  </div>
                </div>

                <div className="px-5 py-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between group-hover:bg-emerald-50 group-hover:border-emerald-100 transition-all">
                  <div className="flex items-center gap-1.5 text-xs text-slate-500">
                    {container.invoice ? (
                      <>
                        <CheckCircle2 className="w-4 h-4 text-green-500" />
                        <span className="font-bold text-slate-700">{container.invoice.invNo}</span>
                      </>
                    ) : (
                      <>
                  ...
                  <div className="p-1.5 bg-white rounded-lg border border-slate-200 group-hover:text-emerald-600 group-hover:border-emerald-200 shadow-sm transition-all">
