...
export default function ContainerSummaryPreview({ summary, containers, onClose }) {
    /* Which financial cols are visible */
    const [visible, setVisible] = useState(
        Object.fromEntries(FINANCIAL_COLS.map(c => [c.key, true]))
    );
    ...
    const toggleAll = (val) =>
        setVisible(Object.fromEntries(FINANCIAL_COLS.map(c => [c.key, val])));

    /* All columns in order */
    const allCols = [
        { key: "no", label: "NO", always: true },
        { key: "containerCode", label: "CONTAINER CODE", always: true },
        { key: "ctn", label: "CTN", always: true },
        { key: "loadingDate", label: "LOADING DATE", always: true },
        { key: "eta", label: "ETA", always: true },
        { key: "dollar", label: "DOLLAR", always: true },
        ...FINANCIAL_COLS,
        { key: "shippingLine", label: "SHIPPING LINE", always: true },
                    ...
                    {showSettings && (
                        <div className="px-6 py-3 border-b border-slate-100 bg-blue-50/50 shrink-0">
                            <div className="flex items-center gap-4 flex-wrap">
                                <span className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] shrink-0">Hide / Show:</span>
