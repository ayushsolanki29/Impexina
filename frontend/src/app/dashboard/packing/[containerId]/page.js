      ...
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-3">
                ...
                <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border transition-colors ${
                  packingList ? 'bg-green-100 text-green-800 border-green-200' : 'bg-amber-100 text-amber-800 border-amber-200'
                }`}>
        ...
        <div className="bg-white rounded-2xl shadow-xl shadow-slate-200/50 border border-slate-200 overflow-hidden mb-8">
          ...
          <div className={`px-6 py-3 flex justify-between items-center border-b ${
            packingList ? 'bg-green-50 text-green-800 border-green-100' : 'bg-slate-50 text-slate-600 border-slate-100'
          }`}>
                  ...
                  <AccordionContent className="pt-2 pb-6">
                    <div className="flex items-start gap-8 pt-4 border-t border-slate-100">
                           ...
                           >
                              {formData.stampImage ? (
                                 <>
                                   <img src={`${process.env.NEXT_PUBLIC_SERVER_URL}${formData.stampImage}`} className="w-full h-full object-contain" alt="Stamp" />
               ...
               <table className="w-full text-sm">
                <thead className="bg-slate-50 border-b border-slate-200">
                    <tr className="text-[10px] font-bold text-slate-500 uppercase tracking-widest leading-none">
                       ...
                       <th className="px-2 py-5 w-24 text-center border-r border-slate-200">Item No.</th>
                    ...
                    {items.map((item, idx) => (
                      <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="px-1 py-1 border-r border-slate-100 text-center">
                          <label className="w-10 h-10 mx-auto flex items-center justify-center border border-dashed border-slate-200 rounded-lg cursor-pointer hover:border-blue-400 transition-all relative overflow-hidden group">
                             {item.photo ? (
                               <img src={`${process.env.NEXT_PUBLIC_SERVER_URL}${item.photo}`} alt="Item" className="w-full h-full object-cover" />
                             ) : (
