...
export default function MumbaiLedgerList() {
  ...
  const duplicateLedger = async (ledgerId, ledgerName) => {
    try {
      const response = await API.post(`/expenses/mumbai-ledger/${ledgerId}/duplicate`, {
  ...
  const deleteLedger = async (ledgerId, ledgerName) => {
    if (!confirm(`Are you sure you want to delete "${ledgerName}"? This action cannot be undone.`)) {
      return;
    }
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className={`text-2xl font-bold flex items-center gap-2 ${
                  stats.balance < 0 ? "text-red-600" : "text-emerald-600"
                }`}>
                ...
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr>
                    <th className="text-left p-4 w-12">
                      <input
                        type="checkbox"
                        checked={selectedLedgers.length === ledgers.length && ledgers.length > 0}
                    ...
                    >
                      ...
                      <td className="p-4">
                        <div
                          ...
                          onClick={() =>
                            router.push(`/dashboard/expenses/mumbai/${ledger.id}`)
                          }
                        >
                          {ledger.name}
                        </div>
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            onClick={() => duplicateLedger(ledger.id, ledger.name)}
                          ...
                          <button
                            onClick={() => archiveLedger(ledger.id, ledger.status !== "ARCHIVED")}
                            className="p-2 text-slate-600 hover:text-orange-600 hover:bg-orange-50 rounded-lg"
                            title={ledger.status === "ARCHIVED" ? "Restore" : "Archive"}
                          >
          ...
          <div className="flex flex-wrap gap-3">
            ...
            {selectedLedgers.length > 0 && (
              <>
                ...
                <button
                  onClick={() => {
                    if (confirm(`Export ${selectedLedgers.length} selected ledgers?`)) {
